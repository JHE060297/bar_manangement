...
export class UsersListComponent implements OnInit {
    ...
    activateUser(user: Usuario) {
        this.userService.activateUser(user.id_usuario).subscribe(
            () => {
                user.is_active = true;
                this.snackBar.open(`Usuario ${user.nombre} activado exitosamente`, 'Cerrar', {
                    duration: 3000
                });
            },
            (error) => {
                this.snackBar.open('Error al activar usuario', 'Cerrar', {
                    duration: 3000
                });
    ...
    deactivateUser(user: Usuario) {
        this.userService.deactivateUser(user.id_usuario).subscribe(
            () => {
                user.is_active = false;
                this.snackBar.open(`Usuario ${user.nombre} desactivado exitosamente`, 'Cerrar', {
                    duration: 3000
                });
            },
            (error) => {
